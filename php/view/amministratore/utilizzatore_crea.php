<div class="input-form">
    <h3>Crea nuovo Utente Utilizzatore</h3>

    <form method="post" action="amministratore/utilizzatore_crea<?= $vd->scriviToken('?')?>">
        <input type="hidden" name="cmd" value="u_nuovo"/>
        ...
        <select name="servizio" id="servizio">
            <?php foreach ($servizi as $servizio) { ?>
                <option value="<?= $servizio->getId() ?>" ><?= $servizio->getNome() ?></option>
            <?php } ?>
        </select>
        <br/>
        ...
        <div class="btn-group">
            ...
            <button type="submit" name="cmd" value="u_nuovo">Salva</button> 
        </div>
    </form>
</div>
